...
一款基于 Tauri 2 的 Windows 桌面工具，用于管理多个 Trae IDE 账号：本地存储、一键切换（改写 IDE 登录态与机器码）、用量查询与统计图表、机器码管理、快速注册与隐私模式写入。
...
如需使用快速注册功能，复制 `.env.example` 为 `.env` 并填写 `VITE_QUICK_REGISTER_API_BASE` / `VITE_APP_ID` / `VITE_APP_SECRET`；未配置时使用代码内的默认兜底值。构建默认不产出安装包（`bundle.active = false`），需要时可在 `src-tauri/tauri.conf.json` 中临时开启。
