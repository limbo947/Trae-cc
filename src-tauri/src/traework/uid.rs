//! TraeWork 账号身份与展示信息解析（权威来源 + 兜底证据链）。
//!
//! **权威来源：解密 `iCubeAuthInfo://icube.cloudide`。** 解出的 JSON 同时给我们两样东西：
//! - `userId` —— 账号身份（也是快照槽名）；
...
pub fn profile_from_storage(storage: &Path) -> Option<AccountProfile> {
    ...
    if let Some(value) = json.get(AUTH_KEY).and_then(|v| v.as_str()) {
        // ① 权威：auth 密文解密后取 userId + 展示字段
        if let Ok(plain) = crate::tc_crypto::decrypt_storage_value(value) {
            if let Some(p) = profile_of(&plain, "auth_blob") {
                return Some(p);
            }
        }
        // ② 兼容旧客户端：该键曾是明文 JSON
        if let Some(p) = profile_of(value, "auth_plaintext") {
            return Some(p);
        }
    }
